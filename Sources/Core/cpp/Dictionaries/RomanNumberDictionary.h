...
/**
	Виртуальный словарь римских чисел базируется на Detail::CRomanArabicConvertor
*/
class CRomanNumberDictionary: public Detail::IBasicMorphologyAnalyser 	  
{
...
private:
   bool IsUnacceptedWord( const wchar_t *wcWord ) const
   {
      APL_ASSERT_PTR(wcWord);

      return wcWord[0] == L'i' && wcWord[1] == L'\0';
   }
   ...
   ) 
	{
      ...
      WordInfo.m_wWord = wcWord;
		...
		WordInfo.m_WordIndex.m_DictIndex.SetDictionaryNumber(DATA_TYPE::NAMES::ednRomanNumeric);
...
public:
	CRomanNumberDictionary() {}
	explicit CRomanNumberDictionary( IAMConverterMorpho *pAMConverter ) { Init(pAMConverter); }
