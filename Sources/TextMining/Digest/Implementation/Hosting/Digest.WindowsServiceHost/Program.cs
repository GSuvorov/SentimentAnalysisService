...
namespace Digest.WindowsServiceHost
{
    static class Program
    {
        static void Main()
        {
	        ...
	        if ( argv.Length >= 2 )
	        {
                ...
                {
                    Console.WriteLine("Usage '[<fullpath>]\\Digest.WindowsServiceHost.exe' -Install [-u]");
                }
	        }
            else
            #endregion
            #region [.RUN.]
            {
                ServiceBase.Run( new DigestWindowsService() );
            }
            #endregion       
        }
    ...
    {
        public static readonly string DISPLAY_SERVICE_NAME = "Digest service (Opinion-mining & Tonality-marking services for AK & XF)";
        public static readonly string SERVICE_NAME         = typeof(DigestWindowsService).Name;
