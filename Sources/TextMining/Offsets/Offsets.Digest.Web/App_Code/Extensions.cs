...
using OffsetsString2XDocumentConverter = global::OffsetsBases.Digest.OffsetsString2XDocumentConverter;
...
namespace Offsets.Digest.Web
{
    internal static class Extensions
    {
        ...
        private const string XSLT_FILENAME_OFFSETS_FLAT      = "Digest.Offsets.Flat.xslt";
        private const string XSLT_FILENAME_OFFSETS_HIERARCHY = "Digest.Offsets.Hierarchy.xslt";
        ...
        private static string MakeOffsetsFlatXslt( this XElement sentence, HttpServerUtility server )
        {
            return (XSLT_FILENAME_OFFSETS_FLAT.MakeXslt( sentence.ToString(), server ));
        }
        private static string MakeOffsetsHierarchyXslt( this XElement sentence, HttpServerUtility server )
        {
            return (XSLT_FILENAME_OFFSETS_HIERARCHY.MakeXslt( sentence.ToString(), server ));
        }
        ...
        private static string MakeOffsetsFlatXslt( this XDocument xdoc, HttpServerUtility server )
        {
            return (XSLT_FILENAME_OFFSETS_FLAT.MakeXslt( xdoc.ToString(), server ));
        }
        private static string MakeOffsetsHierarchyXslt( this XDocument xdoc, HttpServerUtility server )
        {
            return (XSLT_FILENAME_OFFSETS_HIERARCHY.MakeXslt( xdoc.ToString(), server ));
        }
        private static string MakeXslt( this string xsltFilename, XDocument xdoc, HttpServerUtility server )
        {
            ...
            using ( var ms = new MemoryStream() )
            {
                xslt.Transform( xdoc.CreateReader(), null /*xsltArg*/, ms );
