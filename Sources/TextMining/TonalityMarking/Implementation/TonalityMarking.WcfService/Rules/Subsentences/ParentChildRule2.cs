...
{
    internal class ParentChildRule2 : RuleBase
    {
        ...
        {
            public pattern( XElement patternNode, ParentChildRule2 ownerRule )
            {
        ...
        public ParentChildRule2( XElement ruleNode )
            : base( ruleNode )
        {
            ...
            foreach ( var tuple in ParentName_ChildName_Tuples )
            {
                ...
                var pairs = from parent in subsent.Descendants( tuple.Item1/*ParentName*/ )
                            ...
                            let child = parent.ElementsExceptT()
                                              //---.SkipWhile( _ => _.IsElementU() ) //temporary!! (Oleg-bag)
                                              .FirstOrDefault()
                ...
                foreach ( var pair in pairs.Reverse() /*VERY IMPORTANT - in inverse-backward order*/ )
                {
                    ...
                    {
                        #region [.try match.]
                        if ( !pair.PARENT.HasAttributeWithValue( pt.ParentAttributeName, pt.ParentAttributeValue ) )
                            continue;
